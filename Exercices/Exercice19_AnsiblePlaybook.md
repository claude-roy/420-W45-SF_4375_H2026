...
# L'ajout de --break-system-packages permet de ne pas créer d'environnement virtuel.
# Finalement, nous pouvons vérifier le fichier: 
ansible-lint deploy.yaml

# Si vous avez l'erreur : AttributeError 'pathlib' has no attribute '_posix_flavour'.
pip3 install --upgrade ansible-lint --break-system-packages
```

Lorsqu'il n'y a pas d'erreur dans votre playbook, exécutez le playbook.  
...
```bash
ansible-playbook deploy.yaml --tags webapp
```  
...
<details>
<summary>Explication sur l'imbrication des fichiers variables.</summary/>
...
Nous pouvons essentiellement exécuter la commande <code>ansible-vault</code> pour chiffrer n'importe quel fichier. Pour l'instant, la plupart des fichiers que nous allons chiffrer seront des fichiers de variables, car ils contiendront peut-être des mots de passe ou des clés sensibles.
